...
(function () {
  ...
  window.goatcounter.get_data = function (vars) {
    ...
    if (vars.no_session)
      data.ns = typeof vars.no_session === 'function' ? vars.no_session(false) : vars.no_session;
    ...
    var send = function (elem) {
      return function () {
        goatcounter.count({
          ...
          no_session:
            ['1', 't', 'true'].indexOf((elem.dataset.goatcounterNoSession || '').toLowerCase()) !==
            -1,
        });
      };
    };
  ...
  window.goatcounter.visit_count = function (opt) {
    on_load(function () {
      ...
      if (!p) return warn('visit_count: element to append to not found: ' + opt.append);
      p.appendChild(d);
    });
  };
