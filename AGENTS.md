...
4. **Strict CSP, no inline JS.** `script-src 'self'` (GoatCounter's count.js is self-hosted in `public/js/`, ADR-0009). Astro `<script>` blocks are bundled to files; `vite.build.assetsInlineLimit: 0` keeps them external. Server→client data goes through `<script type="application/json">` + `safeJson()`. Never `define:vars`, never `is:inline` JavaScript, never `set:html` with untrusted data.
