...
| `Contact`                        | cards over the Stephan's Quintet backdrop; "Send a signal" card (visible only with JS); copy-email button; `personal-only` cards for leisure links                                                                                                  |
| `ContactSignal`                  | native `<dialog>` as a liquid-glass panel: translucent fill + `backdrop-filter`, 1 px specular rim (masked gradient), slow sheen (transform-only), Crab Nebula portrait with the hook phrase, form, "signal received" state; bottom sheet on phones |
...
| Contact dialog          | centred glass, backdrop blur 14 px + glass blur 28 px, rise / sink, sheen every 14 s | bottom sheet, blur 8 px + 16 px, slide up / down, sheen every 22 s; `prefers-reduced-transparency` → opaque | no animation, opaque under reduced transparency |
...
Verification: `npm run test:e2e` runs the smoke suite in Chromium (desktop, Pixel 7) and WebKit (Desktop Safari, iPhone 14). Playwright's WebKit build on Windows renders without GPU compositing: `backdrop-filter` does not blur (translucent panels show faint content behind them) and Fraunces headings lose their word spacing; both are artefacts of that build, not of Safari, so judge glass and type on a real iPhone or in Chromium. `node scripts/probe-contact.mjs <outdir>` screenshots the contact dialog in four profiles against the dev server + local Worker. The Browser pane's mobile emulation does not deliver IntersectionObserver callbacks reliably and its screenshots can come out black; use a Playwright script with a real device profile instead (the two-engine probe used on 2026-09-06 lives in git history: `test-results/mobile-probe.mjs` pattern).
